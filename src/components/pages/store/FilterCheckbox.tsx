...
const FilterCheckbox = ({title, count, handleClick}: Props) => {
    ...
    return (
        <Field className={"self-stretch inline-flex justify-start items-center gap-3.5"}>
            ...
            <Label className="w-32 justify-start text-sm font-normal font-['Rubik'] text-lime-900 leading-tight">{title}</Label>
            <Description className="w-9 text-right justify-start text-xs font-normal font-['Rubik'] text-lime-900 leading-none">{count}</Description>
        </Field>
    );
};
